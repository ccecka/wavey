...
class FFTInterp_Poles
{
 ...
 private:
  ...
  FFTInterp_Poles( const FFTInterp_Poles& S ) {}
  void operator=( const FFTInterp_Poles& S ) {}
};
...
class FFTInterp_NoPoles
{
 ...
 private:
  // Disable Copy and Assignment
  FFTInterp_NoPoles( const FFTInterp_NoPoles& S ) {}
  void operator=( const FFTInterp_NoPoles& S ) {}
};
...
class FFTInterp_FFT2_Fast
{
 ...
 FFTInterp_FFT2_Fast( Quadrature* q1_, Quadrature* q2_ )
    ...
    T( N_phi * N_theta )
  {
    ...
    for( int n = 0; n < N_rows1; ++n ) {
      int N_phi_n = q1->getRow(n).size();
      fftw_complex* Tn = T0 + n*N_phi;
      rFFT[n] = fftw_plan_dft_1d(N_phi_n,Tn,Tn,FFTW_FORWARD,FFTW_MEASURE);
    }
    ...
    for( int n = 0; n < N_rows2; ++n ) {
      int N_phi_n = q2->getRow(n).size();
      fftw_complex* Tn = T0 + n*N_phi;
      rIFFT[n] = fftw_plan_dft_1d(N_phi_n,Tn,Tn,FFTW_BACKWARD,FFTW_MEASURE);
    }
  }
  ...
  inline void apply( NFunction& A, NFunction& B )
  {
    ...
    for( int n = 0; n < N_rows1; ++n ) {
      complex* Tn = &T[0] + n*N_phi;
      complex* TN = &T[0] + (N_theta1-n)*N_phi;
	...
	//a[k*S] = a[index*S];
	//a[index*S] = 0;
	complex* Tk = &T[0] + k*N_phi;
	complex* Ti = &T[0] + index*N_phi;
	for( int m = 0; m < N_phi0; ++m ) {
	  Tk[m] = Ti[m];
	  Ti[m] = 0;
	}
 ...
 private:
  // Disable Copy and Assignment
  FFTInterp_FFT2_Fast( const FFTInterp_FFT2_Fast& S ) {}
  void operator=( const FFTInterp_FFT2_Fast& S ) {}
};
...
class S2Interp
{
 ...
 private:
  // Disable Copy and Assignment
  S2Interp( const S2Interp& S ) {}
  void operator=( const S2Interp& S ) {}
};
